...
function Men() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setLoadError('')
    fetchProducts({ gender: 'men', limit: 100 }, { signal: controller.signal })
      .then(({ items }) => {
        setMen(items)
        setLoading(false)
      })
  ...
  const subcategories = useMemo(
    () => ['All', ...new Set(men.map((p) => p.subcategory))],
    [men],
  )
  ...
  const visible = useMemo(() => {
    const filtered = sub === 'All' ? men : men.filter((p) => p.subcategory === sub)
    ...
    <main className="bg-ivory text-charcoal">
      <div className="clothza-container py-12 md:py-16">
        ...
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h1 className="type-h1 mt-2">Men</h1>
        ...
        {/* Subcategory filter */}
        <nav aria-label="Filter men by style" className="mt-8 flex flex-wrap justify-center gap-2">
          {subcategories.map((s) => {
            const active = sub === s
            return (
              <button
        ...
        <div className="mt-8 flex flex-col gap-4 border-y border-linen py-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-3">
            <label htmlFor="men-sort" className="type-label whitespace-nowrap">
              Sort by
            </label>
            <select
              id="men-sort"
        ...
        {loading ? (
          ...
          <div className="mx-auto flex w-full max-w-md flex-col items-center gap-4 py-16 text-center">
            ...
            <Link to="/shop?category=men" className="btn btn-secondary">
